...
export function ProfileSelector() {
  ...
  const [profiles, setProfiles] = useState([{ name: "Jordan", initials: "JD", color: "#e50914" }, { name: "Family", initials: "FM", color: "#2b6cb0" }, { name: "Kids", initials: "KI", color: "#7c3aed" }]);
  const [managing, setManaging] = useState(false);
  const enter = (name: string) => { localStorage.setItem("nova-active-profile", name); navigate("/home"); };
  return <main className="profile-selector"><div className="profile-brand"><span className="brand-mark"><Tv size={21} /></span><b>WONDERBOX</b></div><div className="profile-center"><h1>Who's watching?</h1><div className="profile-grid">{profiles.map((profile) => <button key={profile.name} onClick={() => enter(profile.name)} className="profile-card"><span className="profile-avatar" style={{ background: profile.color }}>{profile.initials}</span><strong>{profile.name}</strong>{managing && <small>Choose profile</small>}</button>)}</div><div className="profile-actions"><button onClick={() => setManaging((value) => !value)} className="profile-manage"><Settings size={15} /> {managing ? "Done" : "Manage Profiles"}</button><button onClick={() => setProfiles((current) => [...current, { name: `Guest ${current.length - 2}`, initials: "GU", color: "#0f766e" }])} className="profile-add"><Users size={15} /> Add Profile</button></div></div></main>;
}
...
export function BrowseScreen({ title, kicker, channels = allSeedChannels }: { title: string; kicker: string; channels?: Channel[] }) {
  ...
  const [category, setCategory] = useState(title === "World news" ? "News" : "All");
  const [loading, setLoading] = useState(true);
  useEffect(() => { fetch("https://iptv-org.github.io/iptv/index.m3u").then((response) => response.ok ? response.text() : Promise.reject()).then((raw) => { const parsed = parseM3U(raw); if (parsed.length) { const seeds = [...featuredFallback, ...musicFallback]; setCatalog([...seeds, ...parsed.filter((item) => !seeds.some((seed) => seed.name.toLowerCase() === item.name.toLowerCase()))]); } }).catch(() => undefined).finally(() => setLoading(false)); }, []);
  const filtered = useMemo(() => catalog.filter((channel) => { const matchesCategory = category === "All" || category === "Favorites" ? true : category === "International" ? channel.country === "International" : channel.group.toLowerCase().includes(category.toLowerCase()); const matchesQuery = `${channel.name} ${channel.country} ${channel.group}`.toLowerCase().includes(query.toLowerCase()); return matchesCategory && matchesQuery; }).slice(0, 120), [catalog, category, query]);
  const filters = ["All", "Favorites", "News", "Sports", "Entertainment", "Movies", "Music", "Kids", "Documentary", "International"];
  return <main className="streaming-app"><AppNav /><div className="screen-wrap"><div className="screen-head"><div><p className="eyebrow">{kicker}</p><h1>{title}</h1><span className="catalog-status">{loading ? "Syncing worldwide catalog…" : `${catalog.length.toLocaleString()} channels available`}</span></div><div className="screen-search"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Search ${title.toLowerCase()}`} /></div></div><div className="screen-filters">{filters.map((filter) => <button key={filter} onClick={() => setCategory(filter)} className={category === filter ? "active" : ""}>{filter}</button>)}</div>{filtered.length ? <div className="screen-grid">{filtered.map((channel) => <MiniCard key={channel.id} channel={channel} />)}</div> : <div className="screen-empty"><Search size={22} /><h2>No results</h2><p>Try another search or connect a different public playlist.</p></div>}</div></main>;
}
