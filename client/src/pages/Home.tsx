...
export default function Home() {
  const [channels, setChannels] = useState([...musicFallback, ...fallback]);
  const [active, setActive] = useState(musicFallback[0]);
  ...
  useEffect(() => { fetch(PLAYLISTS.all).then((r) => r.ok ? r.text() : Promise.reject()).then((raw) => { const parsed = parseM3U(raw); if (parsed.length) { const merged = [...musicFallback, ...parsed.filter((candidate) => !musicFallback.some((seed) => candidate.name.toLowerCase() === seed.name.toLowerCase()))]; setChannels(merged); setActive(merged[0]); setSource("IPTV-Org master index + Music TV"); } }).catch(() => toast("Using starter channels while the public index reconnects", { icon: <Radio size={15} /> })).finally(() => setLoading(false)); }, []);
  ...
  return <main className="min-h-screen overflow-x-hidden bg-[#eef1f4] text-[#0d1b2a]"><div className="ambient ambient-one" /><div className="ambient ambient-two" /><nav className="relative z-20 border-b border-[#ffffff]/70 bg-[#eef1f4]/80 backdrop-blur-2xl"><div className="mx-auto flex max-w-[1440px] items-center justify-between px-5 py-4 lg:px-10"><div className="flex items-center gap-8"><a href="#top" className="flex items-center gap-3"><span className="brand-mark"><Tv size={19} /></span><span className="font-display text-xl font-bold tracking-[-0.03em]">NOVA<span className="text-[#536a86]"> IPTV</span></span></a><div className="hidden items-center gap-6 text-sm text-[#0d1b2a]/55 md:flex"><a className="nav-link active" href="#top">Home</a><a className="nav-link" href="#live-tv">All channels</a><a className="nav-link" href="#music-tv">Music TV</a><a className="nav-link" href="#my-list">My List <span className="ml-1 rounded-full bg-[#0d1b2a]/10 px-1.5 py-0.5 text-[10px] text-[#0d1b2a]/60">{favorites.length}</span></a></div></div><div className="flex items-center gap-3"><div className="relative hidden sm:block"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[#0d1b2a]/35" size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search channels" className="h-9 w-[210px] rounded-xl border border-white/80 bg-white/45 pl-9 pr-3 text-xs text-[#0d1b2a] outline-none placeholder:text-[#0d1b2a]/35 focus:border-[#fff]" /></div><button className="icon-button"><Bell size={17} /></button><div className="avatar">JD</div></div></div></nav>
    <div id="top" className="relative z-10 mx-auto max-w-[1440px] px-5 pb-24 pt-8 lg:px-10 lg:pt-12"><section className="hero-grid mb-10 grid gap-7 xl:grid-cols-[1fr_0.72fr] xl:items-end"><div className="relative overflow-hidden rounded-[30px] border border-white/10 bg-[#13151b] p-7 sm:p-10"><div className="hero-backdrop" style={{ backgroundImage: `linear-gradient(90deg, #13151b 0%, rgba(19,21,27,.92) 30%, rgba(19,21,27,.25) 75%, rgba(19,21,27,.5)), url(${HERO})` }} /><div className="relative z-10 max-w-[600px]"><div className="mb-6 flex items-center gap-3 text-xs font-bold uppercase tracking-[0.2em] text-[#ff8876]"><span className="live-dot" /> Live network · {loading ? "syncing" : "ready"}</div><p className="mb-3 text-sm font-semibold uppercase tracking-[0.18em] text-white/40">Now streaming</p><h1 className="font-display text-5xl font-bold leading-[.95] tracking-[-0.06em] text-white sm:text-7xl">Your world.<br /><span className="text-[#cbd9e5]">One remote.</span></h1><p className="mt-6 max-w-[470px] text-sm leading-7 text-white/55">Tune into thousands of public IPTV channels from every corner of the planet. No clutter, no account wall — just pick a signal and press play.</p><div className="mt-8 flex flex-wrap gap-3"><button onClick={() => document.getElementById("player")?.scrollIntoView({ behavior: "smooth" })} className="primary-button"><Play size={16} fill="currentColor" /> Watch live</button><button onClick={() => setPlaylistOpen(true)} className="secondary-button"><Plus size={16} /> Add playlist</button></div></div><div className="hero-scanlines" /></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-4 xl:grid-cols-2">{[{ icon: Radio, value: channels.length > 100 ? "10k+" : channels.length.toLocaleString(), label: "Channels indexed", accent: "coral" }, { icon: Flag, value: "190+", label: "Countries", accent: "blue" }, { icon: Tv, value: "24/7", label: "Live signals", accent: "violet" }, { icon: Zap, value: "Free", label: "Open sources", accent: "yellow" }].map(({ icon: Icon, value, label, accent }) => <div key={label} className={`stat-card accent-${accent}`}><Icon size={17} className="mb-8 opacity-70" /><p className="font-display text-3xl font-bold tracking-[-0.05em] text-white">{value}</p><p className="mt-1 text-xs text-white/40">{label}</p></div>)}</div></section>
