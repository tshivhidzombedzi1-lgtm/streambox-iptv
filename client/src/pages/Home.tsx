...
export function parseM3U(raw: string): Channel[] {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    const name = meta.split(",").slice(1).join(",").trim() || "Untitled channel";
    const slug = name.toLowerCase().normalize("NFKD").replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 52) || "channel";
    result.push({
      id: `iptv-${i}-${slug}`,
      name,
